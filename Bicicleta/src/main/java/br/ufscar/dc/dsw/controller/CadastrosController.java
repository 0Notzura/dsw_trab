...
public class CadastrosController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        Usuario usuario = (Usuario) request.getSession().getAttribute("usuarioLogado");// PEGA O USUAIO PASSADO NA LINHA
                                                                                       // 38 DO INDEX
        Erro erros = new Erro();
    ...
    /*
     * private void apresentaFormCadastro(HttpServletRequest request,
     * HttpServletResponse response)
     * throws ServletException, IOException {
     * request.setAttribute("clientes", getCliente());
     * RequestDispatcher dispatcher =
     * request.getRequestDispatcher("/logado/cadastro/formulario.jsp");
     * dispatcher.forward(request, response);
     * }
     */
    ...
    private void insere(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            UsuarioDAO usuarioDAO = new UsuarioDAO();// CRIA UM NOVO OBJ USUARIO Q POSSUI OS CAMPOS (nome, login, senha,
                                                     // papel)
            Usuario usuario = usuarioDAO.getbyLogin(login);// RETORNA O USUARIO COM BASE NO LOGIN

            Cadastros cadastro = new Cadastros(dma, horario, locadora, usuario);// CRIA O OBJ LOGADO

            Cadastros existente = dao.get(usuarioLogado.getCpf(), locadora.getCnpj(), dma);//
